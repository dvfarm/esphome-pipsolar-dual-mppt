...
void Pipsolar::loop() {
    ...
    switch (this->used_polling_commands_[this->last_polling_command_].identifier) {
      case POLLING_QPIRI:
        ...
        if (this->charger_source_priority_solar_first_switch_) {
          this->charger_source_priority_solar_first_switch_->publish_state(value_charger_source_priority_ == 1);
        }
        if (this->charger_source_priority_solar_utility_switch_) {
          this->charger_source_priority_solar_utility_switch_->publish_state(value_charger_source_priority_ == 2);
        }
        if (this->charger_source_priority_solar_switch_) {
          this->charger_source_priority_solar_switch_->publish_state(value_charger_source_priority_ == 3);
        }
  ...
  if (this->state_ == STATE_POLL_COMPLETE) {
    if (this->check_incoming_crc_()) {
      ...
      ESP_LOGD(TAG, "STATE_POLL_CHECKED");
      return;
    } else {
      this->state_ = STATE_IDLE;
    }
